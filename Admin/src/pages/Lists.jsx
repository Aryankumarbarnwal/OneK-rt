...
function Lists() {
  ...
  const removeList = async (id)=>{
    try {
      ...
      if(result.data){
        toast.success("1 Products Deleted");
        fetchList()
      }
      else{
  ...
  return (
    ...
    from-[#141414] to-[#0c2025] text-[white]'>
      <Nav/>
      <div className='w-[100vw] h-[100%] flex items-center justify-start'>
        <Sidebar/>

        <div className='w-[82%] h-[100%] lg:ml-[320px] md:ml-[230px] mt-[70px]
         flex flex-col gap-[30px] overflow-x-hidden
         py-[50px] ml-[100px]'>
          ...
          list?.length > 0 ? (
            list.map((item, index) =>(
              <div className='w-[90%] md:h-[120px] h-[90px]
              bg-slate-600 rounded-xl flex items-center justify-start gap-[5px]
              md:gap-[30px] p-[10px] md:px[30px] ' key={index}>
                ...
                <div className='w-[10%] h-[100%] bg-transparent flex items-center justify-center'>
                  ...
                  rounded-md md:hover:bg-red-300 md:hover:text-black cursor-pointer ' onClick={()=>removeList(item._id)}>X</span>
                </div>
